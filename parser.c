...
void help(void) 
{
	...
	puts("-b <str>: tag di inizio marcatura del riferimento (before)");
	puts("-a <str>: tag di fine marcatura del riferimento (after)");
	puts("-m <htmlnir|htmlris|dtdnir|urn>: marcatura richiesta (impostazione automatica di -a -b):");
	puts("   - htmlnir: tag HTML per accedere al risolutore di NormAttiva:");
	puts("              -b \"<a href=\"http://www.normattiva.it/uri-res/N2Ls?__URN__\">\"");
	puts("              -a \"</a>\"");
	puts("   - htmlris: tag HTML per accedere al risolutore indicato nel parametro -u:");
	puts("              -b \"<a href=\"<url-risolutore>__URN__\">\"");
	puts("              -a \"</a>\"");
	...
	puts("-r <nif>: tipo di riferimenti:");
	puts("   - n:  segnalazione dei riferimenti non completi (solo dtdnir)");
	puts("   - i:  marcatura dei riferimenti interni (solo dtdnir)");
	puts("   - f:  inserisce in commento partizioni del frammento non convertite in ID (solo dtdnir)");
