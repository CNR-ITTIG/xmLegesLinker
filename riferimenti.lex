...
{LET}{S}[a-z][a-z]?{LAT}?\)?	{ BEGIN(sudd); salvaPos(); 
				yylval=(int)strdup(utilConvLettera(yytext)); return LETTERA; }
...
{DECRETO}({ST}del{ST})?{PRES}({ST}del{ST})?{CONS}({ST}dei{ST})?{MINIS}	{ BEGIN(atto); 
					salvaPos();
...
{DIR}({ST}del{ST})?{PRES}({ST}del{ST})?{CONS}({ST}dei{ST})?{MINIS}	{ BEGIN(atto); 
					salvaPos(); 
...
{ODZ}({ST}del{ST})?{PRES}({ST}del{ST})?{CONS}({ST}dei{ST})?{MINIS}	{ BEGIN(atto); 
					salvaPos(); 
...
<atto>{NUM}?{S}({UENUM_NA}|{UENUM_AN})		{ salvaPos(); yylval=(int)strdup(yytext); 
						return UE_NUM; }
...
<sudd,atto>(del(la|lo)?{SPA}(cit(\.|at[ao])|medesim[ao]|stess[ao]|predett[ao]))	{ pos+=yyleng; 
									return CITATO; }
...
<atto>{N12}[/\.-]{N12}[/\.-]({N4}|{N12})/[^0-9]	{ salvaPos();
						yylval=(int)utilConvDataNumerica(yytext); 
						return DATA_GG_MM_AAAA; }

<atto>{N12}{ST}{MESI}{ST}({N4}|{N12})/[^0-9]	{ salvaPos(); 
						yylval=(int)utilConvDataEstesa(yytext); 
						return DATA_GG_MM_AAAA; }
